...
class GS2(nn.Module):
    def __init__(self, scale_factor, cr = 0.25):
        ...
        # )
        self.block1 = nn.Sequential(
            nn.Conv2d(1,int( (1 + 64) * self.cr ),kernel_size = 1,stride=1),
            nn.BatchNorm2d( int( (3 + 64) * self.cr ) ),
            nn.PReLU(),

            nn.Conv2d(int( (1 + 64) * self.cr ), int( (1 + 64) * self.cr ),kernel_size = (1,9),stride=1,padding=(0,4)),
            nn.BatchNorm2d( int( (3 + 64) * self.cr ) ),
            nn.PReLU(),

            nn.Conv2d( int( (1 + 64) * self.cr ) , int( ( 1 + 64) * self.cr ) ,kernel_size = (9,1),stride=1,padding=(4,0)),
            nn.BatchNorm2d(int( (3 + 64) * self.cr )),
            nn.PReLU(),

            nn.Conv2d( int( (1 + 64) * self.cr ), 64,kernel_size = 1,stride=1),
            # nn.BatchNorm2d(34),
            nn.PReLU()
        )
        ...
        block8.append(nn.Conv2d(64, int((64+1) * cr), kernel_size=1, stride=1))
        block8.append(nn.BatchNorm2d(int((64+1) * cr)))
        block8.append(nn.PReLU())
        block8.append(nn.Conv2d(int((64+1) * cr), int((64+1) * cr), kernel_size=(1,9), padding=(0,4)))
        block8.append(nn.BatchNorm2d(int((64+3) * cr)))
        block8.append(nn.PReLU())
        block8.append(nn.Conv2d(int((64+1) * cr), int((64+1) * cr), kernel_size=(9,1), padding=(4,0)))
        block8.append(nn.BatchNorm2d(int((64+1) * cr)))
        block8.append(nn.PReLU())
        block8.append(nn.Conv2d(int((64+1) * cr), 1, kernel_size=1, stride=1))
