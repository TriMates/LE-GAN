...
from mdl4.stage1_student import DS1_FULL as DS1
...
parser.add_argument('--card', type=str, default='1',help='select GPU:0 or 1')

parser.add_argument('--outf', default='./result/dis/teachernetv7.36re_1215/output', help='folder to output images and model checkpoints')
parser.add_argument('--ckpt', default='./result/dis/teachernetv7.36re_1215/ckpt',help='ckpt file path')
...
gs1sd = torch.load("./result/dis/teachernetv7.36re_1215/ckpt/netG1_epoch_3760.pth")
gs2sd = torch.load("./result/dis/teachernetv7.36re_1215/ckpt/netG2_epoch_3760.pth")
ds1sd = torch.load("./result/dis/teachernetv7.36re_1215/ckpt/netD1_epoch_3760.pth")
ds2sd = torch.load("./result/dis/teachernetv7.36re_1215/ckpt/netD2_epoch_3760.pth")
gS1.load_state_dict(gs1sd)
gS2.load_state_dict(gs2sd)
dS1.load_state_dict(ds1sd)
dS2.load_state_dict(ds2sd)
gS1 = gS1.cuda()
gS2 = gS2.cuda()
dS1 = dS1.cuda()
dS2 = dS2.cuda()
...
generator_criterion = GeneratorLoss(100, 1, 5)
...
def setGLoss(visual = 1, adv = 0.001, perc=0.006, tv = 2e-8):
    generator_criterion.visual = visual
    generator_criterion.adv = adv
    generator_criterion.perc = perc
    generator_criterion.tv = tv
    return


def pretrain():
    for epoch in range(3761, opt.niter, 1):
        for i,data in enumerate(dl):
            ...
            err_D1_ = (errD1_real + errD1_fake) * 0.5
            ...
            setGLoss(visual = 100, adv=1, perc=5)
            errG1 = 1 * nn.L1Loss()(fake1, GFT1_) + generator_criterion(DF1, fake1, real)
            ...
            # del GFT1
            setGLoss(visual = 100, adv=1, perc=5)
            errG2 = 1 * nn.L1Loss()(fake2, GFT2_) + generator_criterion(DF2, fake2, real)
    ...
    torch.save(gS1.state_dict(), opt.ckpt+'/netG1_Final5000.pth')
    torch.save(dS1.state_dict(), opt.ckpt+'/netD1_Final5000.pth')
    torch.save(gS2.state_dict(), opt.ckpt+'/netG2_Final5000.pth')
    torch.save(dS2.state_dict(), opt.ckpt+'/netD2_Final5000.pth')
